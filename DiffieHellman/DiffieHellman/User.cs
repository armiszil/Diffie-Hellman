using System;
using System.Collections.Generic;
using System.Globalization;
using System.IO;
using System.Numerics;
using System.Security.Cryptography;

namespace Diffiehellman
{
    public class User:IDisposable
    {
        AesManaged AES = new AesManaged();
        public static Random rnd = new Random();
        BigInteger privateKey;
        public BigInteger PrivateKey { get { return privateKey; } }
        string name;
        BigInteger generatedPublicKey;
        public BigInteger GeneratedPublicKey { get { return generatedPublicKey; } }
        string Key;
        byte[] IV;
        byte[] RecviedIV;
        public byte[] I_V { get { return IV; } }
        private  readonly byte[] Salt =
             new byte[] { 10, 20, 30, 40, 50, 60, 70, 80 };

        public User(string name)
        {
            this.name = name;
            //Generate PrivateKey a || b
            privateKey = GeneratePrime();
            IV = AES.IV;

        }
        

        private  byte[] CreateKey(string password, int keyBytes = 32)
        {
            const int Iterations = 300;
            var keyGenerator = new Rfc2898DeriveBytes(password, Salt, Iterations);
            return keyGenerator.GetBytes(keyBytes);
        }


        public byte[] SendMyMessage(string message)
        {

            byte[] encrypted;
            // Create a new AesManaged.    
            using (AesManaged aes = new AesManaged())
            {
                
                // Create encryptor    
                ICryptoTransform encryptor = aes.CreateEncryptor(CreateKey(this.Key), IV);
                // Create MemoryStream    
                using (MemoryStream ms = new MemoryStream())
                {
                    // Create crypto stream using the CryptoStream class. This class is the key to encryption    
                    // and encrypts and decrypts data from any given stream. In this case, we will pass a memory stream    
                    // to encrypt    
                    using (CryptoStream cs = new CryptoStream(ms, encryptor, CryptoStreamMode.Write))
                    {
                        // Create StreamWriter and write data to a stream    
                        using (StreamWriter sw = new StreamWriter(cs))
                            sw.Write(message);
                        encrypted = ms.ToArray();
                    }
                }
            }
            return encrypted;   
        }

        private BigInteger P()
        {
            
            var input = "";
            var hugeNumber = BigInteger.Parse(input, NumberStyles.HexNumber);
            BigInteger p = hugeNumber;
            return p;
        }

        public void Recieve(byte[] encrypted)
        {
            //Recieve
            byte[] encrypted_Message = encrypted;

            //Decrupt

            string plaintext = null;
            // Create AesManaged    
            using (AesManaged aes = new AesManaged())
            {
                
                // Create a decryptor    
                ICryptoTransform decryptor = aes.CreateDecryptor(CreateKey(this.Key), RecviedIV);
                // Create the streams used for decryption.    
                using (MemoryStream ms = new MemoryStream(encrypted_Message))
                {
                    // Create crypto stream    
                    using (CryptoStream cs = new CryptoStream(ms, decryptor, CryptoStreamMode.Read))
                    {
                        // Read crypto stream    
                        using (StreamReader reader = new StreamReader(cs))
                            plaintext = reader.ReadToEnd();
                    }
                }
            }

            Console.WriteLine(this.name +" :    "+ plaintext);
        }



        static public int GeneratePrime()
        {
            // Prime Generator
            bool prime = false;
            int num = 0;
            while (prime == false)
            {
                num = rnd.Next(1, 100);
                prime= isPrime(num);
            }
            return num;
        }


        public void Generate_Key()
        {

            //Generate Public keys
            BigInteger g = 2;
           
            generatedPublicKey = BigInteger.ModPow(g,privateKey,P());
        }


        public void ExchangeKeys(BigInteger exch,byte[] IV)
        {

             Key = (BigInteger.ModPow(exch,privateKey, P())).ToString();
           
            this.RecviedIV = IV;
        }


         public static bool isPrime(int n)
        {
            // Corner cases
            if (n <= 1)
            {
                return false;
            }
            if (n <= 3)
            {
                return true;
            }

            // This is checked so that we can skip middle five numbers in below loop
         
            if (n % 2 == 0 || n % 3 == 0)
            {
                return false;
            }

            for (int i = 5; i * i <= n; i = i + 6)
            {
                if (n % i == 0 || n % (i + 2) == 0)
                {
                    return false;
                }
            }

            return true;
        }

        protected virtual void Dispose(bool disposing)
        {
            if (disposing)
            {
                if (this.AES != null)
                    this.AES.Dispose();

            }
        }
        public void Dispose()
        {
            Dispose(true);
            GC.SuppressFinalize(this);
        }
    }
}
